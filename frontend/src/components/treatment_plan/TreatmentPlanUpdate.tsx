...
function TreatmentCreate() {
    ...
    const [treatment_detail, setTreatment_detail] = React.useState('');
    const [treatment_explain, setTreatment_explain] = React.useState<string>("");

    const dentist2 = [{ID: 1, FirstName: "หมอยา"}];
    const defaultValue2 = {ID: 1, FirstName: dentist2.find(d => d.ID === 1)?.FirstName || ""};
    const [firstt, setFirstt] = React.useState<string>();
    

    
    const firsttAsString = ''+(dentist.find(d => d.ID === 1)?.FirstName);
    const fff = "you" + firstt;
    console.log("firsttAsString")
    //console.log(firsttAsString)
    console.log("firsttAsString")
        ...
        fetch(`http://localhost:3001/treatment_plans/${id}`)
            ...
            .then((res) => {
                if (res.data) {
                    ...
                    setFirstt(dentist.find(d => d.ID === 1)?.FirstName);
        ...
        <Container maxWidth="md">
            ...
            <Paper>
                ...
                <Grid item xs={6}>
                    <FormControl fullWidth variant="outlined">
                        ...
                        <Autocomplete
                            ...
                            onChange={(e, value) => {
                                ...
                                console.log(dentist.find(d => d.ID === 1)?.FirstName);
                                console.log(treatment_explain);
                                console.log("value firstname")
                                treatment_plan.DentistID = value?.ID
                            }}

                            defaultValue={{ ID: 1 ,FirstName: dentist.find(d => d.ID === 1)?.FirstName}}
                            
                            renderInput={(params) => <TextField {...params} 
                                label="เลือกทันตเเพทย์" />}
                        />
                    </FormControl>
                </Grid>
                    ...
                    {/* <Grid item xs={6}>
                        <FormControl fullWidth variant="outlined">
                            <p className="good-font">Godfather</p>
                            <Autocomplete
                                id="combo-box-demo"
                                options={top100Films}
                            ...
                            <Autocomplete
                                ...
                                renderInput={(params) => (
                                    <TextField
                                        ...
                                        // defaultValue={dentist.find(d => d.ID === 1)?.FirstName}
                                        // value={dentist.find(item => item.ID === 1)?.FirstName || ''}
                                         //defaultValue='หมอเทวดา' variant="outlined"
